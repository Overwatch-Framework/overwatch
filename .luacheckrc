--- Overwatch Lua Formatting & Linting Rules (.luacheckrc)

-- Tell Luacheck to recognize Garry's Mod and Overwatch globals
std = "luajit+gmod+overwatch"
max_line_length = 128

-- Allow defined globals (e.g., for modules)
allow_defined_top = true

-- Ignore unused arguments named "_" (e.g., for iterators)
ignore_args = { "^_" }
...
-- Enforce spacing (can't check inside parentheses, manual review needed)
-- But warn on missing whitespace around operators, etc.
read_globals = { "require" }

-- Show code context in warnings
codes = true

-- Disable some rules that conflict with our framework style
-- e.g., we allow unused functions if used via hooks
unused = {
    args = false,
    secondaries = false
}

-- Global variables used across Overwatch framework
stds.overwatch = {}
stds.overwatch.globals = {
    "ow",
    "Schema",
