...
local function GetViewModelBob(pos, ang)
    ...
    Multiplier = Lerp(time, Multiplier, ply:IsSprinting() and swayMultSprint or swayMult)
    ...
    local d = math.Clamp(ViewModelBobVelocity / 350, 0, 1)
    ...
    if ( ply:IsSprinting() ) then
        time = ft
        SprintInertia = Lerp(time, SprintInertia, 1)
        WalkInertia = Lerp(time, WalkInertia, 0)
    else
        time = ft / 4
        SprintInertia = Lerp(time, SprintInertia, 0)
        WalkInertia = Lerp(time, WalkInertia, 1)
    end

    if ( SprintInertia > 0 ) then
        local SprintMultiplier = Multiplier * SprintInertia
    ...
    if ( WalkInertia > 0 ) then
        local WalkMultiplier = Multiplier * WalkInertia
        pos = pos - (ang:Up() * math.sin(BobCT * step) * 0.1 * ((math.sin(BobCT * 3.515) * 0.2) + 2) * mag * crouchmult * WalkMultiplier) - (ang:Up() * SideMove * -0.05) - (ang:Up() * smoothjumpmove2 * 0.2)
